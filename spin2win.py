...
def master(lotto_type, **sort_criteria): 
    data = collect_data(LOTTO_CONST[lotto_type]["FILENAME"])
...
def charter(lotto_type, sorted_df):
    df = pd.DataFrame({'roll1': roll1, 'roll2':roll2, 'roll3': roll3, 'roll4': roll4, 'roll5': roll5})
    print("\n---DF : 1 COL PER ROLL---\n", df)
    df = df.astype(float)
